...
def atmospheric_density(temperature=20*u.deg_C, pressure_pa=100000*u.Pa, 
    ...
    atmosphere_values = {'R':R, 'Z':Z, 'Ma':Malpha, 'Mw':Mw, 'svp':svp, 'f':f,\
                         'density':density, 'TT_C':TT_C, 'TT_K':TT_K, \
                         'pressure_pa':pressure_pa, 'humidity':humidity, 'xw':xw}
...
#    input_times.delta_ut1_utc = 0.0 # set offset = 0 
    ...
    if num_waves == 1: refrac = np.array([refrac])
    ...
    with erfa_astrom.set(ErfaAstromInterpolator(1440 * u.min)):
        ...
        frame_TETE = TETE(obstime=input_times, location=haleakala)
...
#    alt_all[idx] = alt_all[idx - 1]
    ...
    #get everything in degrees         # HA off by 1.2 min?
    parallactic_angle_sin = np.clip(np.sin(np.deg2rad(ha_all)) / \
                                    np.sin(np.deg2rad(90 - alt_all)) * \
                                    np.sin(np.deg2rad(90 - latitude.to(u.deg).value)),-1,1)
...
def offsets(wavelength=np.array([400, 500, 600, 700, 800])*u.nm, 
    ...
    refraction_mag = refraction_atm[0]['refraction_mag (arcsec)']
...
if __name__ == "__main__":
    ...
    wavelengths  = np.array([400,525,630,700,850,1083,1525])*u.nm     # nm
    num_waves    = len(wavelengths)
    refrac = refractivity(wavelengths,temp,pressure,humidity,co2,verbose=0)
    
    times_all_1d = Time('2020-01-01 00:00:00',scale='utc') + np.arange(1440*366) * u.min
    ...
    if do_haleakala == 1:
        ...
        hours_ut_hka       = ((times_all_hka[:1440].value - times_all_1d[0].value) * 24) % 24 
...
#        alt_all[idx] = alt_all[idx - 1]
        ...
        altitude_mask = np.where(np.tile(alt_hka_2d.reshape(365,1440,1),(1,1,num_waves))>=4,1,0)
...
#        for wv in range(num_waves):
#            print("\tCalculating for wavelength {:.0f} nm".format(wavelengths[wv].value))
        dispersion_temp = atmospheric_refraction(wavelengths, times_all_hka,\
            air_pressure=pressure, air_temp=temp, humidity=humidity,co2_conc=co2,\
            latitude=latitude,longitude=longitude,altitude=altitude,verbose=0)
        dispersion_1d = np.squeeze(dispersion_temp[0]['refraction_mag (arcsec)'])
        dispersion_hka     = np.reshape(dispersion_1d, (365, 1440, num_waves))
        dispersion_hka     = dispersion_hka * altitude_mask + (1 - altitude_mask) * (-1)
#            if wv == 0:
        parang_hka = np.reshape(dispersion_temp[0]['parallactic_angle (degrees)'],(365, 1440))
        ...
        parashift_hka_caII = dispersion_hka[:,:,np.argmin(np.abs(wavelengths - 397*u.nm))]\
                           - dispersion_hka[:,:,np.argmin(np.abs(wavelengths - 854*u.nm))]
        ...
        angle_extra = np.arange(-45,45).reshape((1,1,-1))
...
#            if ((j+1) % 80) == 0: print(j+1)
            angle_curve      = parang_hka_big[:,j+tstart:]
            dispersion_curve = np.clip(np.abs(parashift_big[:,j+tstart:]),None,50)
            angle_tangent = np.clip(np.abs(np.sin((np.tile(angle_curve, (1,1,num_angs)) - angle_curve[:,0].reshape((365,1,1)) + angle_extra)*u.deg)),None,2)
...
#            perp_shift_conv = convolve(perp_shift,kernel)
            ...
            lim_alt_idx = (np.tile(altitude_curve, (1,1,num_angs)) <= 3).argmax(axis=1)
            min_shift = np.minimum(lim_slit_idx, lim_alt_idx)
            time_available_hka[:,j+tstart] = min_shift[:,zero_ang]
            time_available_hka_rot[:,j+tstart] = angle_curve[np.arange(365),lim_slit_idx[:,zero_ang],0]
            ...
            time_available_hka_opt[:,j+tstart,1] = maxpos
        ...
        output = {'times_all_hka_2d': times_all_hka_2d, 'hours_ut_hka': hours_ut_hka, \
                  'wavelengths': wavelengths, 'alt_hka_2d': alt_hka_2d, \
                  'az_hka_2d': az_hka_2d, 'ha_hka_2d': ha_hka_2d, 'parang_hka': parang_hka, \
                  'dispersion_hka': dispersion_hka, 'time_available_hka': time_available_hka, \
                  'time_available_hka_rot': time_available_hka_rot, \
                  'time_available_hka_opt': time_available_hka_opt}
        oname='refraction.calc.haleakala.' + date_string + '.pkl'
        with open(oname,'wb') as f:
            pickle.dump(output,f)
        print("Results saved in file: " + oname)
